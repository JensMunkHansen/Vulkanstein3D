...
void drawSprite(int spriteNum, float distWall, float widthf, float heightf) {
    ...
    if(insideX && insideY && validZBuffer) {
        ...
        float d = (heightf - vMoveScreen) - SceneData.screenSize.y * 0.5 + spriteData.spriteHeight * 0.5;
...
void main() {
    xdata data = res[int(uvCoord.x * SceneData.screenSize.x)];
    float heightf = float(SceneData.screenSize.y) * uvCoord.y;
    ...
    if(data.draw.x <= heightf && heightf <= data.draw.y) {
        ...
        if(data.draw.y < 0)
            data.draw.y = SceneData.screenSize.y;
        ...
        vec4 tex = isCeil ? SceneData.floorTex : SceneData.ceilTex;
        if(tex.a == 0.0f) {
            FragColor = vec4(tex.rgb, 1.0f);
        } else {
            float currentDist;
            if(isCeil)
                currentDist = float(SceneData.screenSize.y) / (2.0f * (float(SceneData.screenSize.y) - heightf) - float(SceneData.screenSize.y));
            else
                currentDist = float(SceneData.screenSize.y) / (2.0f * heightf - float(SceneData.screenSize.y));
            ...
            vec2 currentFloor = vec2(
                weight * data.floorWall.x + (1.0f - weight) * SceneData.position.x,
                weight * data.floorWall.y + (1.0f - weight) * SceneData.position.y
            );
    ...
    heightf = float(SceneData.screenSize.y) * uvCoord.y;
    float widthf = float(SceneData.screenSize.x) * uvCoord.x;
    for(int spriteNum = 0; spriteNum < SceneData.spriteCount; spriteNum += 1) {
