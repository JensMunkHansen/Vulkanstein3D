...
void main() {
    ...
    uint w = SceneData.screenSize.x;
    int height = SceneData.screenSize.y;
    ivec3 pixel = ivec3(0, 0, 0);
    ivec2 mapSize = imageSize(map);

    // x-coord in camera space
    float cameraX = 2 * float(x) / float(w) - 1;
    vec2 rayDir = SceneData.direction + SceneData.plane * cameraX;

    // where we are now (the box from the map)
    ivec2 mapPos = ivec2(SceneData.position);
    ...
    if(rayDir.x < 0) {
        step.x = -1;
        sideDist.x = (SceneData.position.x - mapPos.x) * deltaDist.x;
    } else {
        step.x = 1;
        sideDist.x = (mapPos.x + 1.0f - SceneData.position.x) * deltaDist.x;
    }

    if(rayDir.y < 0) {
        step.y = -1;
        sideDist.y = (SceneData.position.y - mapPos.y) * deltaDist.y;
    } else {
        step.y = 1;
        sideDist.y = (mapPos.y + 1.0f - SceneData.position.y) * deltaDist.y;
    }
    ...
    float perpWallDist;
    if(side == 0) {
        perpWallDist = (mapPos.x - SceneData.position.x + (1.0f - step.x) / 2.0f) / rayDir.x;
    } else {
        perpWallDist = (mapPos.y - SceneData.position.y + (1.0f - step.y) / 2.0f) / rayDir.y;
    }
    ...
    float wallX;
    if(side == 0) {
        wallX = SceneData.position.y + perpWallDist * rayDir.y;
    } else {
        wallX = SceneData.position.x + perpWallDist * rayDir.x;
    }
    wallX -= floor(wallX);
