...
void Debug2DStage::record(const FrameContext& ctx)
{
  ...
  vk::DescriptorSet ds;
  if (m_graph.material_set_count() > 0)
  {
    int idx = std::clamp(*m_material_index, 0,
      static_cast<int>(m_graph.material_set_count()) - 1);
    ds = m_graph.material_descriptor_set(ctx.frame_index, static_cast<uint32_t>(idx));
  }
  else
  {
    ds = m_graph.default_descriptor_set(ctx.frame_index);
  }
