...
// Scalar Fresnel-Schlick (needed for iridescence layer interfaces)
...
// Vec3 Fresnel-Schlick for iridescence (without f90 parameter)
vec3 F_Schlick_irid(vec3 f0, float cosTheta) {
    float x = clamp(1.0 - cosTheta, 0.0, 1.0);
    float x2 = x * x;
    float x5 = x * x2 * x2;
...
// Core thin-film interference evaluation
vec3 evalIridescence(float outsideIOR, float eta2, float cosTheta1, float thinFilmThickness, vec3 baseF0) {
    ...
    float R12 = F_Schlick_scalar(R0, cosTheta1);
    float R21 = R12;
    float T121 = 1.0 - R12;
    float phi12 = 0.0;
    ...
    vec3 R23 = F_Schlick_irid(R1, cosTheta2);
    vec3 phi23 = vec3(0.0);
...
// Energy-conserving per-channel blend for iridescence
// Replaces scalar alpha blend with per-channel Fresnel blend
vec3 rgb_mix(vec3 base, vec3 layer, vec3 rgb_alpha) {
    return base * (vec3(1.0) - rgb_alpha) + layer * rgb_alpha;
}
