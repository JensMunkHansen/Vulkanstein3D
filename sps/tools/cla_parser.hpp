...
public:
  ...
  CommandLineArgumentTemplate(std::string argument, bool takes_args)
    ...
    , m_takes_values(takes_args)
  {
  }

  [[nodiscard]] const std::string& argument() const { return m_argument; }

  [[nodiscard]] bool takes_values() const { return m_takes_values; }
};
...
public:
  explicit CommandLineArgumentValue(std::string value)
...
class CommandLineArgumentParser
{
  ...
  std::optional<CommandLineArgumentTemplate> make_arg_template(
...
public:
  ...
  /// @return `std::nullopt` if argument `name` wasn't specified by the user at all, `true` if the
  /// argument was
  ///          specified but doesn't take any values and the first value of the argument lexically
  ///          casted to type `T`.
  template <typename T>
  ...
  [[nodiscard]] std::size_t parsed_arg_count() const { return m_parsed_arguments.size(); }
};
