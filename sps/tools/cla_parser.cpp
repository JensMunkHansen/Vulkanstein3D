...
std::optional<CommandLineArgumentTemplate> CommandLineArgumentParser::make_arg_template(
...
// TODO: Take in an std::vector<std::string> of arguments? This would avoid any hard to debug out of
// bounds errors (with
...
void CommandLineArgumentParser::parse_args(int argc, char** argv)
{
  for (int i = 1; i < argc; i++)
  {
    ...
    // We have to fail if the user doesn't specify a value for an argument expecting one (since it
    // would mess up the rest of parsing).
    if (i++ >= argc)
    {
